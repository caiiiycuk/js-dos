...
For example, one for sound drivers another for game it self. It can be done with [`extractAll`](https://js-dos.com/6.22/docs/api/generate.html?page=js-dos-fs#dosfs-extract-all) method. It accepts array of [`sources`](https://js-dos.com/6.22/docs/api/generate.html?page=js-dos-fs) to extract and extract them in one time. Actually, `extract` method is just syntax shugar and implemented via `extractAll`. 
