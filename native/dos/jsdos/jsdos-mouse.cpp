...
#include "pic.h"
#include "inout.h"
#include "../dosbox/src/ints/int10.h"
#include "bios.h"
#include "dos_inc.h"
...
#define MOUSE_IRQ 12
#define POS_X (static_cast<Bit16s>(mouse.col) & mouse.gran_x)
#define POS_Y (static_cast<Bit16s>(mouse.row) & mouse.gran_y)
...
mickey getRelMickey(float prevCol, float prevRow,
  ...
  if (mickey_x >= 32768.0)  {
    mickey_x -= 65536.0;
  } else if (mickey_x <= -32769.0) {
    mickey_x += 65536.0;
  }

  if (mickey_y >= 32768.0) {
    mickey_y -= 65536.0;
  } else if (mickey_y <= -32769.0) {
    mickey_y += 65536.0;
  }
...
void Mouse_CursorMoved(float xrel,float yrel,float x,float y,bool emulate) {
	if (emulate) {
		...
		if (CurMode->type == M_TEXT) {
			...
			if ((mouse.max_x > 0) && (mouse.max_y > 0)) {
				mouse.col = x*mouse.max_x;
				mouse.row = y*mouse.max_y;
			} else {
				mouse.col += xrel;
				mouse.row += yrel;
			}
	...
	if (!useps2callback) {		
		if (mouse.col > mouse.max_x) mouse.col = mouse.max_x;
		if (mouse.col < mouse.min_x) mouse.col = mouse.min_x;
		if (mouse.row > mouse.max_y) mouse.row = mouse.max_y;
		if (mouse.row < mouse.min_y) mouse.row = mouse.min_y;
	} else {
		if (mouse.col >= 32768.0) mouse.col -= 65536.0;
		else if (mouse.col <= -32769.0) mouse.col += 65536.0;
		if (mouse.row >= 32768.0) mouse.row -= 65536.0;
		else if (mouse.row <= -32769.0) mouse.row += 65536.0;
	}
...
static void Mouse_Reset(void) {
	...
	mouse.col = static_cast<float>((mouse.max_x + 1)/ 2);
...
//	LOG(LOG_MOUSE,LOG_NORMAL)("MOUSE: %04X %X %X %d %d",reg_ax,reg_bx,reg_cx,POS_X,POS_Y);
	...
	case 0x04:	/* Position Mouse */
		...
		if ((Bit16s)reg_cx >= mouse.max_x) mouse.col = static_cast<float>(mouse.max_x);
		else if (mouse.min_x >= (Bit16s)reg_cx) mouse.col = static_cast<float>(mouse.min_x);
		else if ((Bit16s)reg_cx != POS_X) mouse.col = static_cast<float>(reg_cx);
	...
	case 0x07:	/* Define horizontal cursor range */
		{	//lemmings set 1-640 and wants that. iron seeds set 0-640 but doesn't like 640
			...
			if(mouse.col > mouse.max_x) mouse.col = mouse.max_x;
			if(mouse.col < mouse.min_x) mouse.col = mouse.min_x;
                        if(mouse.mickeyCol > mouse.max_x) mouse.mickeyCol = mouse.max_x;
                        if(mouse.mickeyRow < mouse.min_x) mouse.mickeyRow = mouse.min_x;
	...
	case 0x08:	/* Define vertical cursor range */
		{	// not sure what to take instead of the CurMode (see case 0x07 as well)
			...
			if(mouse.row > mouse.max_y) mouse.row = mouse.max_y;
			if(mouse.row < mouse.min_y) mouse.row = mouse.min_y;
                        if(mouse.mickeyRow > mouse.max_y) mouse.mickeyRow = mouse.max_y;
                        if(mouse.mickeyRow < mouse.min_y) mouse.mickeyRow = mouse.min_y;
			/* Or alternatively this:
	...
	case 0x0b: { /* Read Motion Data */
          ...
          mouse.mickeyCol = rel.col;
          mouse.mickeyRow = rel.row;
        } break;
...
static Bitu INT74_Handler(void) {
	if (mouse.events>0 && !mouse.in_UIR) {
		...
		if (mouse.sub_mask & mouse.event_queue[mouse.events].type) {
			...
			reg_cx=POS_X;
			reg_dx=POS_Y;
			reg_si=static_cast<Bit16s>(rel.mickey_x);
			reg_di=static_cast<Bit16s>(rel.mickey_y);
			...
			DoPS2Callback(mouse.event_queue[mouse.events].buttons, static_cast<Bit16s>(mouse.col), static_cast<Bit16s>(mouse.row));
		} else {
