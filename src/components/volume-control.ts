...
export function VolumeControl(props: ControlsProps) {
    ...
    useEffect(() => {
        ...
        const onEnd = (ev: PointerEvent) => {
            ...
            setVolume(volume);
            // TODO: apply volume
            // props.player().setSensitivity(minVolume + volume);
            ev.stopPropagation();
            ev.preventDefault();
        };
    ...
    const top = (100 - Math.max(0, Math.min(100, Math.round(volume / volumeRange * 100)))) + "%";
                ...
                <div ref=${scrollRef} class=${"relative sensitivity cursor-pointer rounded-2xl bg-gray-400 w-2 " +
                    ...
                    <div class="flex flex-row items-center absolute pointer-events-none -mt-3" style=${{ top }}>
                        ...
                        <div class="bg-green-100 ml-2 py-1 px-2 rounded z-50 ${"opacity-" + opacity}">
                            ${Math.round((minVolume + volume) * 10) / 10}
                        </div>
                    </div>    
                </div>
            </div>
